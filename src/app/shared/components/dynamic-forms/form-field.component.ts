...
@Component({
    ...
    template: `
        <div class="du-form-control w-full">
            <label class="du-label" [for]="inputId()">
                <span class="du-label-text">{{ label() }}</span>
            </label>
            <input [id]="inputId()" [type]="type()" [class]="getInputClasses()" [placeholder]="placeholder()" [formControl]="control()" />
...
export class FormFieldComponent {
    ...
    readonly inputId = computed(() => `field-${this.label().toLowerCase().replace(/\s+/g, '-')}`);

    readonly validationErrors: Record<string, (error: ValidationErrors) => string> = {
